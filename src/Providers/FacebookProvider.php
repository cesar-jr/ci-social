...
class FacebookProvider extends AbstractProvider
{
    ...
    protected function getUserByToken($token)
    {
        ...
        return $this->getUserByOIDCToken($token) ?? $this->getUserFromAccessToken($token);
    }
    ...
    protected function getUserByOIDCToken($token)
    {
        ...
        if ($data['iss'] !== 'https://www.facebook.com') {
    ...
    protected function getPublicKeyOfOIDCToken(string $kid)
    {
        ...
        $key = array_find(json_decode($response->getBody()->getContents(), true)['keys'], fn($key) => $key['kid'] === $kid);
